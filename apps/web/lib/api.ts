...
/**
 * `instanceof FormData` is not reliable here. This module runs in the browser
 * AND in the Node server render, and a `FormData` built in one realm fails the
 * check in the other — the exact class of bug that makes an upload silently
 * arrive as the string `[object FormData]`. Duck-typing the two methods the
 * runtime needs is both realm-safe and honest about what is being asked.
 */
function isFormData(body: unknown): body is FormData {
  if (typeof FormData !== 'undefined' && body instanceof FormData) return true;
  const b = body as { append?: unknown; getAll?: unknown } | null | undefined;
  return typeof b?.append === 'function' && typeof b?.getAll === 'function';
}

export type RequestOptions = {
  method?: 'GET' | 'POST' | 'PATCH' | 'PUT' | 'DELETE';
  /**
   * Request body. A `FormData` is passed to `fetch` untouched (multipart);
   * a string is sent as-is; anything else goes through `JSON.stringify`.
   */
  body?: unknown;
...
export async function api<T>(path: string, opts: RequestOptions = {}): Promise<T> {
  ...
  // FormData carries its own `multipart/form-data; boundary=…`, generated by
  // the runtime. Setting Content-Type here would send a boundary-less header
  // the server cannot parse, which is why the four upload screens used to
  // bypass `api()` entirely — and inherited undici's five-minute timeout,
  // `ApiError` handling and the 401 cache purge as four separate omissions.
  const multipart = isFormData(opts.body);
  if (opts.body !== undefined && !multipart && !headers['Content-Type']) {
