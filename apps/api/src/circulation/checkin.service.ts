...
export class CheckinService {
    ...
    const outcome = await client.$transaction(
      async (tx) => {
        ...
        const result: CheckinResult = {
          loanId: loan.id,
          itemId: loan.item.id,
          disposition: dispositionFor(loan.item, branchId),
  ...
  private async probe(
    tx: TxV2,
    input: CheckinInput,
  ): Promise<{ itemId: string; patronId: string | null }> {
    const itemId =
      input.itemId ??
      (
    ...
    return { itemId, patronId: loan?.patronId ?? null };
  }
...
/**
 * What the desk should do with the copy, DERIVED.
 *
 * Phase 16 can reach two of the four values. `hold_shelf` needs phase 17's
 * queue and `transit` needs phase 23's routing, and both are declared in
 * `CHECKIN_DISPOSITIONS` rather than added later so a client does not have to
 * be redeployed to understand a returned book.
 */
