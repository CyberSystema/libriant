...
> **RECONCILED AGAINST THE DIVERGENCE LOG THROUGH PHASE 20l (2026-09-16).** This
> document was written before any of it existed, and building it proved parts of
> it wrong. Corrections are marked **_Amended_** inline, next to the text they
> correct, rather than collected at the end — a reader acting on a paragraph
> needs the correction in that paragraph. The measurement behind each one is in
> the divergence log, `README.md`, which is the running record; this file is the
> plan.
>
> Every one of the 26 phase entries in the log has been read back against this
> file and the outstanding corrections folded in — 58 of them, each verified to
> still be outstanding and to quote this document verbatim before it was
> applied. That check mattered: 19 further proposals were rejected because the
> plan already reflected them, and re-applying a correction would have been a
> worse defect than leaving one stale, because the next reader trusts this file.
>
> **What this reconciliation does NOT mean.** It is not a claim that the plan is
> now correct — only that it no longer contradicts what has been measured up to
> phase 20l. Phases 21 onward have not been built, and everything they describe
> is still a plan written by somebody who had not tried it.
>
...
CREATE TABLE marc_record_contents (
  ...
  source_normalization text,                    -- 'nfc'|'nfd'|'mixed' — LOWER CASE, classified from what ARRIVED
...
**Leader write rules, corrected.** The bibliographic spec had these backwards and would have emitted records that Koha, Alma, Voyager and `yaz-marcdump` reject. The rule is asymmetric: **on read**, honour whatever /20-23 declares and use those widths to slice the directory, recording an anomaly if it is not `4500`; **on write**, always emit `/10='2'`, `/11='2'`, `/20-23='4500'`, set `/09` from the encoding being written, and recompute `/00-04` and `/12-16`. `/09` is set from the encoding of the bytes being produced, never copied from the source — a UTF-8 record emitted with a blank `/09` is mojibake at the far end — and **that rule binds the store write, not only the export**: the canonical leader keeps positions 5..11, so `/09` is inside `content_hash`. A record stored with the source's own `/09 = ' '` — every Greek ABEKT and Aleph MARC-8 export — hashes `c28e3d69cc9bc8b0…` in the store and `fd682577fc5ef1f2…` after export and re-parse. `marc_records.charset_code` holds that same byte and must agree with the stored leader. The original leader bytes survive only in `source_blob`. `005` is stamped inside the write transaction on every content change and excluded from `content_hash`, so a save that changes nothing writes no version but an export always carries a current transaction timestamp. _Amended: the stamp cannot be taken from the wall clock. MARC 005 has TENTHS-of-a-second resolution and a complete write transaction was measured at **1.60 ms**, so ten consecutive edits produced ten IDENTICAL stamps — phase 10's "two consecutive edits produce strictly increasing 005" does not merely risk a collision under this write path, it fails essentially always. Worse, the obvious test passes on a wall-clock stamper, because two supertest PATCHes are usually more than 100 ms apart; the test must be in-process, ten edits back to back, reading the stamps from the STORED document. The shipped stamper is `max(clockTick, previous + 0.1 s)`, whose deliberate consequence is that a burst of edits pushes 005 ahead of real time — and ahead of `updated_at` — by a tenth of a second each._ _Amended: the stamp cannot be taken from the wall clock. MARC 005 has TENTHS-of-a-second resolution and a complete write transaction was measured at **1.60 ms**, so ten consecutive edits produced ten IDENTICAL stamps — phase 10's "two consecutive edits produce strictly increasing 005" does not merely risk a collision under this write path, it fails essentially always. Worse, the obvious test passes on a wall-clock stamper, because two supertest PATCHes are usually more than 100 ms apart; the test must be in-process, ten edits back to back, reading the stamps from the STORED document. The shipped stamper is `max(clockTick, previous + 0.1 s)`, whose deliberate consequence is that a burst of edits pushes 005 ahead of real time — and ahead of `updated_at` — by a tenth of a second each._ `Leader/05` is derived from status (`n` on create, `c` on edit, `d` on delete/merge). `008/00-05` is derived from `created_at` and never rewritten — every "titles added this year" statistic and the ISO 2789 return depend on it.
...
_Amended after the UTC-session interlude (between phases 17 and 18)._ Three schema-wide facts that `timestamptz(3)` alone does not give you, enforced by `check:session-timezone`. **The session must be UTC, and it is pinned twice**: `ALTER DATABASE … SET TimeZone TO 'UTC'` at every `CREATE DATABASE` site before the migrations, and `options: '-c timezone=UTC'` on all three Prisma pools (one definition, in `packages/shared`; never on the connection URL). `@prisma/adapter-pg` writes a `timestamptz` as a naive local timestamp and reads one back by declaring the local wall clock to be UTC, so on any other session the instant on disk is wrong by the session's offset while Prisma still agrees with itself. **`DEFAULT now()` in the DDL below is belt-and-braces, not the writer**: Prisma materialises `@default(now())` client-side and names the column in every INSERT, so all 49 `DEFAULT CURRENT_TIMESTAMP` columns are dead code on the Prisma path and nothing in 2.0 may rely on "the database stamps this". **Partition bounds on a `timestamptz` key are emitted as explicit `TIMESTAMPTZ '… +00'` literals**, and a table whose existing bounds are not midnight UTC is refused rather than extended — a bare bound literal is resolved in the creating session's zone, and the test is midnight UTC rather than a `+00` suffix, because a UTC session renders an Athens-created bound as `'2026-08-31 21:00:00+00'` too. A `date` partition key (`circulation_statistics`) carries no zone and is left alone. `@db.Date` is unaffected: the adapter builds a date from `getUTCFullYear/Month/Date` and its read normaliser is the identity.
...
CREATE TABLE branches (
  ...
  timezone text NOT NULL REFERENCES iana_timezones(name) ON DELETE RESTRICT,
                                    -- THE circ-5 fix, enforced in the DATABASE. It cannot be a CHECK:
                                    -- a subquery is refused outright (0A000) and an IMMUTABLE wrapper
                                    -- over pg_timezone_names costs 11.1 ms/row (111 s per 10,000 inserts
                                    -- against 4.6 ms unconstrained) and is a false IMMUTABLE anyway.
                                    -- The FK costs 8.2 us and raises 23503. `iana_timezones` is seeded
                                    -- from pg_timezone_names MINUS ('Factory','posixrules') — the only
                                    -- two names Intl.DateTimeFormat refuses, and the one row on which two
                                    -- Postgres 16.15 builds disagreed (597 rows, identical on both).
                                    -- App-layer validation is not enough: phase 19's copy-forward is
                                    -- PL/pgSQL and writes `branches` without going through TypeScript.
...
CREATE TABLE bib_records (
  ...
  projection_anomalies jsonb NOT NULL DEFAULT '[]',  -- INVENTED in 11a: every judgement the total projector declines to make, 13 codes, each with the tag and a sentence addressed to a cataloguer
...
`call_number_sort` is produced by `@libriant/shared/callnumber` and is **pure ASCII, fixed-width, zero-padded** (`"005.13300000|KER|2019|C02"`). Tenant databases are created with ICU `el-GR` as the default collation (`--locale-provider=icu --icu-locale=el-GR --locale=C.UTF-8`, so `datcollate` reads `C.UTF-8` while every text comparison still goes through ICU el-GR); a non-ASCII sort key reorders under that collation — the same trap as perf-13 — and shelf order must be byte-identical in Postgres, in the browser and in the offline Rust wand.
...
-- AMENDED after building phase 13: there is NO resolve index. Measured at the
-- 500-rule acceptance size the planner never chose
-- `(priority DESC, specificity DESC, id) WHERE enabled`, and forcing it read
-- 403 buffers against 5 for a sequential scan plus sort, because a whole-table
-- read through a btree is random-order heap access. Ranking happens in
-- `resolve.ts` over the loaded snapshot (`compareRank`, a TOTAL order on
-- distinct ids, so the winner does not depend on input order — asserted by
-- reversing the array before sorting), and the snapshot loader therefore has no
-- `orderBy` at all. What ships is a LISTING index, for the one place SQL order
-- reaches a human — the matrix listing, `/circulation/explain`'s neighbours, a
-- phase-26 report — because a UI that lists rules in a different order than the
-- engine ranks them is the same bug wearing a different hat:
...
Holds are title / volume / item level with `hold_policy_id` + `policy_snapshot` pinned identically, `holds_one_assignment_per_item` making a double-assigned item impossible, and a **targeted** queue rebalance (`WHERE queue_position > <vacated>`). The 1.0 blanket `> 0` decrement is correct only because the head always leaves; with suspended holds being skipped it would move the skipped reader to position 0, which `holds_position_is_one_based CHECK (queue_position IS NULL OR queue_position >= 1)` refuses — so the old form does not corrupt the queue, it ABORTS, and the named regression test asserts SQLSTATE `23514` rather than a multiset of positions, because a wrong multiset can be produced by several subtly different implementations and `23514` only by one that tried to seat a waiting reader at position 0.
...
so block recalculation is an `INSERT … ON CONFLICT DO UPDATE` and a sweep racing a desk transaction settles in Postgres instead of aborting the librarian's checkout — the DATA-1 lesson applied to blocks. Two separate things live in that sentence: the INDEX is what makes duplication impossible (zero dupes even for `DELETE`+`INSERT`), and the `ON CONFLICT` is what makes the librarian's transaction survive. It holds only under two conditions, both measured at 25 concurrent sweeps against one patron, 20 iterations. **The desk's pin on the patron must be `pg_advisory_xact_lock`, never `SELECT … FOR UPDATE`**: `patron_blocks.patron_id` references `patrons`, so every genuine block INSERT runs the FK check, which takes a `FOR KEY SHARE` tuple lock on the patron row, and that plus a desk `FOR UPDATE` on the same row deadlock — 0/20 desk commits and ZERO loans written. An advisory lock does not participate in the FK row-lock graph at all; if a row lock on `patrons` is ever genuinely wanted it must be `FOR NO KEY UPDATE` (20/20 desk, 500/500 sweeps). **And every writer must pin `ReadCommitted` explicitly**: at REPEATABLE READ, `ON CONFLICT DO UPDATE` onto a concurrently-updated row raises `40001` and the desk aborts 15/15. The `ON CONFLICT` inference must also IMPLY the partial index predicate — dropping or weakening the `WHERE` is `42P10`, and naming it as a constraint is `42704`, because a partial unique INDEX is not a CONSTRAINT. Two separate things live in that sentence: the INDEX is what makes duplication impossible (zero dupes even for `DELETE`+`INSERT`), and the `ON CONFLICT` is what makes the librarian's transaction survive. It holds only under two conditions, both measured at 25 concurrent sweeps against one patron, 20 iterations. **The desk's pin on the patron must be `pg_advisory_xact_lock`, never `SELECT … FOR UPDATE`**: `patron_blocks.patron_id` references `patrons`, so every genuine block INSERT runs the FK check, which takes a `FOR KEY SHARE` tuple lock on the patron row, and that plus a desk `FOR UPDATE` on the same row deadlock — 0/20 desk commits and ZERO loans written. An advisory lock does not participate in the FK row-lock graph at all; if a row lock on `patrons` is ever genuinely wanted it must be `FOR NO KEY UPDATE` (20/20 desk, 500/500 sweeps). **And every writer must pin `ReadCommitted` explicitly**: at REPEATABLE READ, `ON CONFLICT DO UPDATE` onto a concurrently-updated row raises `40001` and the desk aborts 15/15. The `ON CONFLICT` inference must also IMPLY the partial index predicate — dropping or weakening the `WHERE` is `42P10`, and naming it as a constraint is `42704`, because a partial unique INDEX is not a CONSTRAINT.
...
`fee_types` · `patron_accounts` · **`fees`** · `account_transactions` · `account_entries` · `fee_allocations` · `payment_methods` · `cash_drawer_sessions` · `receipts` · `receipt_number_counters` · `ledger_discrepancies`

_Amended after building phase 18: eleven tables, not fourteen._ **`cash_drawer_movements` is designed away, not deferred.** Every cash movement is already an `account_entries` row on `cash_on_hand` whose transaction names the session, so the expected total is `opening_float_cents + SUM(debit_cents − credit_cents)`; a movements table would be a SECOND recording of the same fact, and a cash count is worth taking only because it is an INDEPENDENT check on the journal. **`fee_payment_intents` moves to phase 33 and `payment_terminals` to phase 34** (`refunds_payable` lands with the first of them): phase 18 takes money at a desk with a librarian present, so there is no intent to hold and no device to speak to, and a wrong guess in a PCI-adjacent table is worse than an absent one. The chart of accounts is an **enum**, not a `ledger_accounts` table — a tenant that can delete an account is a fee that posts nowhere; what a library configures is which of three revenue labels a `fee_type` posts to.
...
CREATE TABLE fees (
  ...
  archived_at timestamptz(3),                    -- 19a: 1.0's soft delete; NOT status='cancelled'
  -- The ONE balance predicate a reader may ask. Two readers disagreed before it
  -- existed: patrons.service.ts summed WHERE outstanding_cents > 0, the checkout
  -- gate summed WHERE closed_at IS NULL, and a CANCELLED charge made them differ.
  owed_cents bigint GENERATED ALWAYS AS
...
That partial unique preserves the 1.0 `INSERT … ON CONFLICT … DO UPDATE` upsert verbatim (DATA-1), which is why a return racing the accrual sweep does not abort the librarian's transaction — and with `timestamptz` the `NOW() AT TIME ZONE 'UTC'` workaround finally becomes a plain `now()`. Payments are a real double-entry ledger, and `SUM(debit) = SUM(credit)` per transaction is a **database law, not a report**: `account_entries_balance` is a STATEMENT-level `AFTER INSERT` trigger over the transition table, so an unbalanced journal cannot commit. The side effect is the point — a journal must be posted in ONE insert, which makes `postJournalWithin` the only way to write an entry. The `DEFERRABLE INITIALLY DEFERRED` constraint-trigger form is also a law and is refused: it fires at COMMIT, so a mistake in the fee module would abort the librarian's CHECKIN with a stack trace pointing at the commit. The trigger body resolves names through `TG_TABLE_SCHEMA`, never a pinned `search_path`, because a pinned path stores the schema NAME and phase 20's `ALTER SCHEMA lbr2 RENAME TO public` invalidates it. On top of that law, the nightly `ledger-reconcile` covers the identities one insert cannot see and emits `libriant_circ_ledger_drift_total`, which **alerts rather than self-heals** — a self-healing reconciler hides the bug that caused the drift.
...
`trace` always returns `{ matchedRuleId, beatenRuleIds[], selectorsUsed, wildcardsUsed, calendarRolls[] }`, where `beatenRuleIds` is the rules that **matched and lost** — not every rule of lower rank, which returns 499 ids from a 500-rule snapshot on every checkout, allocated on the hot path and rendered into an explain screen nobody can read. "Your branch rule beat the tenant default" is what the librarian asking _why is this due on the 19th_ was after, where `beatenRuleIds` is the rules that **matched and lost** — not every rule of lower rank, which returns 499 ids from a 500-rule snapshot on every checkout, allocated on the hot path and rendered into an explain screen nobody can read. "Your branch rule beat the tenant default" is what the librarian asking _why is this due on the 19th_ was after, which powers `GET /t/:slug/circulation/explain` and the "Why is this due 19 Sep?" tooltip. `PolicySnapshotService` caches process-locally, versioned by the trigger-maintained `circulation_policy_version` counter, invalidated over Redis pub/sub with a 30 s TTL backstop, and **never fails open to a default policy** — a wrong loan period is a wrong receipt. The same ranking function resolves notice templates (branch 2, category 1). Consumers: circulation, holds, fees, notices, OPAC, SIP2, NCIP, the offline Rust core (pinned to `fixtures/resolution-vectors.json`, run by both `node --test` and `cargo test`).
...
| ISO 2709                              | 2008                                  | B    | 7     | Reader honours the declared entry map; **writer always emits /10='2', /11='2', /20-23='4500'**, recomputes /00-04 and /12-16, refuses a field over 9,999 bytes, a record over 99,999, and a separator byte inside a value, each naming MARCXML — the **field** limit is the one real catalogues hit (a multi-volume 505 contents note reaches it), so the `catalog_marc` export carries an `oversize.xml` member beside `catalogue.mrc` and a `manifest.json` in which `total = inCatalogueMrc + inOversizeXml`, always.                                                                                                                                                                                                                                                                                                                           |
...
| MARC-8                                | LC codetables.xml                     | B    | 7     | **G0/G1 designations reset at every field boundary.** Mappings are string-valued (some map to sequences). Double-diacritic halves U+FE20–FE23 span two base characters and get their own pass before the general combining-mark reorder. _Amended: **two** of the twelve graphic sets ship — Basic Latin and Extended Latin (ANSEL), the two DEFAULT designations. There is no Greek, Cyrillic, Hebrew, Arabic or EACC table: LC's `codetables.xml` is not in this repository and was not reachable, and hand-typing one would put an uncheckable transcription into the product sold to Greek libraries. An unsupported designation raises `marc8-unsupported-charset`, substitutes U+FFFD and leaves the original bytes in `source_blob`. `pnpm marc8:tables` generates the rest from `vendor/marc/codetables.xml` the day somebody vendors it._ |
...
> **Amended after building eighteen of them.** The rule held for most and not for all: 9 shipped as 9a with the rest folded into 12–15 and 18, 10 as 10a + 10b, 11 as 11a + 11b, 19 as 19a + 19b, and 20 as 20a + 20b-i + 20b-ii + 20b-iii — and then further still: the importer's 2.0 write path came out of 20b-ii as **20c**, circulation history as **20d**, and the log's phase-20 series continues past them. Phases 7 and 8 each needed a follow-up defect commit. And `check:all` is a weaker promise than this sentence implies: it runs the static gates, typecheck and lint, but **no test suite, no build, and not `format:check`**, and `check:schema-drift` and `check:image-size` run only inside CI. `main` was red for eleven phases while every local signal was green — see the divergence log.
...
**5. Metrics registry + queue consolidation.** `apps/api/src/observability/metrics.registry.ts` with `defineMetric({name, help, type, labels, alert})` as the single declaration site; `check:alerts` rewritten to read declarations and to assert **both** directions; `SOURCES` in `scripts/check-alerts.ts` left at the roots that exist — `apps/protocol-gateway/src` and a Rust declaration file are not in this repository (`apps` is api, desktop, site, web; there is no `Cargo.toml`), and a scan root that matches nothing reads as coverage while covering nothing, which is the failure mode this gate exists to prevent. `SOURCES` is the one place to widen when the gateway lands at phase 60, and a declaration whose `source` has no entry there fails the build; the FIVE queues that exist (`email-outbox`, `scheduled`, `import`, `maintenance`, `export`) consolidated into one list in `apps/api/src/queues/consumers.ts`, from which all four `worker.ts` surfaces (healthz map, readyz `handlesUp`, metrics, shutdown) are derived, per REL-04 — the six queues §1 names arrive with the milestones that produce them (`notices` is phase 22), and registering a consumer for a queue nothing produces would pin `/readyz` at 503 permanently, because readiness is now strictly "every registered consumer is running"; `tenant-pool-budget.ts` extended.
_Depends on:_ 2.
...
_Depends on:_ 3.
_Accept:_ An ESLint rule fails on `fetch(` inside a screen component, **and a second rule fails on importing the legacy `api()` helper there** — `api()` is not `fetch`, so a `fetch(`-only selector is satisfied by a screen that never touches the port, and phases 20j and 20k shipped six such calls between them under a green rule. The import ban is `no-restricted-imports` (not a second `no-restricted-syntax` entry, which would silently disable the first one's selectors), scoped to the directories already repointed — `catalog/**` first — with zero exemptions, and grown one directory at a time, because 104 files still import `api` and 100 of them are 1.0 screens the cutover deletes. The port boundary is wider than `fetch` alone: the same block also bans `window.open`, `navigator.clipboard`, `navigator.onLine`, the `online`/`offline` event listeners and any literal containing the `/lbr-api` rewrite prefix — each has a port method, and the prefix is one host's rewrite rather than part of the API contract. `check:openapi` passes against an empty registry and fails on a deliberately drifted fixture.
...
**7. `packages/marc` — the codec.** Types; ISO 2709 reader (lifted from `import/parsers/marc-parser.ts` with its three defects fixed — structure discarded, indicators dropped, subfield code read past buffer end) and writer with the corrected leader rules; MARCXML reader + hand-rolled writer; MARC-in-JSON; MARC-8 both directions over two HAND-WRITTEN default tables (Basic Latin, Extended Latin/ANSEL) with per-field designation reset and string-valued multi-character mappings; LC's `codetables.xml` is not in this repository, so `pnpm marc8:tables` generates the remaining sets only once somebody vendors it at `vendor/marc/codetables.xml`, and until then `--check` skips with a printed reason and every other designation raises `marc8-unsupported-charset`; NFC/NFD policy; MARC path grammar; `applyOps` with `$6`/880 occurrence renumbering as a post-condition; `diff`; canonical hash excluding 005; NACO + Greek normalize re-exported from `@libriant/shared`. `scripts/gen-marc8-tables.ts`. Uses WebCrypto, no `node:` built-ins, so it runs in a webview.
_Depends on:_ 1.
_Accept:_ Property test over ≥5,000 records (_amended: the corpus is **generated**, not real — this repository has no real MARC and the authoring session had no way to obtain any. `generateCorpus(n)` emits ISO 2709 bytes from an independent hand-written emitter parameterised by per-exporter quirk profiles, which is what makes the round-trip meaningful: two independent implementations agreeing, rather than one agreeing with itself_): `parse(serialize(parse(b))) ≡ parse(b)` for all, and `serialize(parse(b)) === b` byte-for-byte on **100 %** of conforming records with zero tolerance; field-for-field idempotence on all 5,000 records including the malformed ones; and every non-conforming record asserting the specific reason it cannot be reproduced. A percentage over a corpus this repository generates is tunable to any number by editing one probability, so the tolerance is replaced rather than reported. Measured: only leader positions 0-4, 12-16 and 20-23 — the ones the writer is required to recompute or fix — ever change. Every serialized leader matches `^.{10}22.{8}4500$` — the `.{9}` form is 25 characters wide (10 + 2 + 9 + 4) for a 24-byte leader, because positions 12-19 are eight characters, not nine, and no conforming record can match it. MARC-8 ships the two DEFAULT graphic sets only — Basic Latin and Extended Latin (ANSEL), reached with no escape sequence — because LC's `codetables.xml` is not in this repository and was not reachable; any other designation raises `marc8-unsupported-charset`, substitutes U+FFFD so the damage is VISIBLE, and leaves the original bytes in `source_blob`, which is recoverable in a way a hand-typed table is not. In place of "round-trips LC's codetable vectors", four table-free checks each rule out a class of transcription error: every combining byte maps to something Unicode itself calls a mark; every byte decodes and re-encodes to itself; every combining target is inert under NFC and NFD; and the two sets occupy disjoint byte ranges. The designation-reset vector is a 245 with ANSEL designated into G0 followed by a Latin 260 — the identical mechanism, failing if the reset is removed, but explicitly NOT the Greek vector. `pnpm marc8:tables` closes this the day somebody puts LC's file at `vendor/marc/codetables.xml`. `applyOps` inserting a paired field maintains the invariant BOTH `$6` policies end at: every `$6` pairs bijectively, no occurrence number is used twice, and `00` is untouched. The DEFAULT is `repair` — allocate the lowest free number, break collisions, never touch a correct link — not wholesale renumbering: LC assigns the occurrence number at random and says it exists to match associated fields rather than to sequence them, so a compacting pass on an imported record (whose numbers arrive sparse) rewrites `$6` on the very first save, floods the diff and breaks the byte round-trip this same phase is measured on. The plan's literal behaviour survives as the `compact` policy with the plan's literal test. `check:greek-folding` still green.
...
_Depends on:_ 7.
_Accept:_ Zero ISSUES of any severity across all 5,000 records of the phase-7 corpus. There are no LC records in this repository; the corpus is not a substitute for real ones but is not circular either — it was written to test byte-level round-tripping, its tags were chosen to exercise a serializer, and it predates this definition. "Zero ERRORS" would have been nearly vacuous, because with `coverage.confidence: "transcribed"` no table rule CAN produce an error; the tightened form is proved to bite by deleting the LCSH value from 650's indicator-2 list (5,000 hits) and by marking 650 non-repeatable (2,544 errors). A record with a repeated 245, an illegal 6XX ind2, an obsolete 440 and a 3-char indicator each produce exactly one precisely-worded issue naming tag, occurrence and rule. `validateDelta` returns an empty blocking set on a neutral edit to a record with 12 pre-existing errors. `check:marc-schema` fails on a hand-edited definition and on a template binding a tag absent from its definition.

**9. The 2.0 baseline migration.** _(Amended: shipped as **9a** — seventeen tables, not the ~219 §3 names: the ten §3 specifies with a full `CREATE TABLE` block, five skeletons forced by their foreign keys, `audit_log`, and one support table. Notices, bookings, `custom_field_values` and the settings singletons were not in it; they arrived with phases 12–18 or are still deferred. `prisma/schema-v2/BASELINE-SCOPE.json` names every table §3 mentions with a status each and is the authority on which of them exist, and `check:schema-conventions` fails if that manifest and the schema disagree — so the gap is enumerated rather than lost.)_ One squashed migration creating the cross-domain tables in §3 — branches (with the cycle trigger, ISIL, **timezone**), service points, shelving locations, calendars, item/material types, patron categories, patrons + cards, holdings, items, the MARC store trio, loans, holds, fees + ledger, notice tables, bookings, roles/permissions, `change_events` + generated triggers, partitioned `audit_log`, `sync_client_changes`, `custom_field_values`, settings singletons — plus `btree_gist`, `btree_gin`, `record_version_seq`, `default_toast_compression=lz4`. **No services yet.** `check:changelog-coverage` and `check:schema-conventions` land here.
_Depends on:_ 2, 8.
...
**10. MARC store: write path, versions, locks.** `apps/api/src/bib` — the single `write()` (advisory lock → hash precondition → `applyOps` → NFC → 005 stamp → `validateDelta` → hash → version → audit) — _amended: this arrow sequence is a trap read left to right. Taking the advisory lock after the read that produced the expected hash, and comparing the hash in JS, was measured to give **exactly the protection of no lock at all**: 2 winners / 3 versions at 2-way and 25 / 26 at 25-way, identical to the naive unconditional UPDATE, because both readers finish before either lock is requested. `pg_advisory_xact_lock('bib:<id>')` must be the FIRST statement of the transaction, before any read, and the precondition must be a SQL compare-and-swap (`UPDATE … WHERE content_hash = $expected`), folded into the same statement as the `row_version` bump — two UPDATEs on `marc_records` fire the changelog trigger twice and every consumer then processes the edit twice. BOTH mechanisms are kept, at READ COMMITTED: the CAS cannot prevent the `40P01` that two writers touching `marc_record_contents` and `marc_records` in opposite orders produce, the lock cannot detect staleness, and raising the isolation level instead hands the loser `40001` — a retry-shaped error carrying neither the current record nor the diff the acceptance clause demands._ — _amended: this arrow sequence is a trap read left to right. Taking the advisory lock after the read that produced the expected hash, and comparing the hash in JS, was measured to give **exactly the protection of no lock at all**: 2 winners / 3 versions at 2-way and 25 / 26 at 25-way, identical to the naive unconditional UPDATE, because both readers finish before either lock is requested. `pg_advisory_xact_lock('bib:<id>')` must be the FIRST statement of the transaction, before any read, and the precondition must be a SQL compare-and-swap (`UPDATE … WHERE content_hash = $expected`), folded into the same statement as the `row_version` bump — two UPDATEs on `marc_records` fire the changelog trigger twice and every consumer then processes the edit twice. BOTH mechanisms are kept, at READ COMMITTED: the CAS cannot prevent the `40P01` that two writers touching `marc_record_contents` and `marc_records` in opposite orders produce, the lock cannot detect staleness, and raising the isolation level instead hands the loser `40001` — a retry-shaped error carrying neither the current record nor the diff the acceptance clause demands._, record locking in Postgres with heartbeat/expiry/take-over, version list/diff/restore. _Amended: the record lock is a different mechanism at a different timescale from the write path's advisory lock — a human's intent to hold a record open for ten minutes — and shipped separately as **10b**. It is ADVISORY: `write()` does not read it and must not learn to, because an import, an overlay, a merge, a batch job and the phase-19 copy-forward all have to be able to write a record a cataloguer has open, and a lock that could refuse a save is one somebody has to override at 2am. `marc_record_locks` is one row per record ever opened, with liveness in the acquire predicate rather than an index predicate (Postgres refuses a `WHERE expires_at > now()` partial unique as non-IMMUTABLE, and a design where a sweep is what makes a lapsed lock acquirable freezes every record whose editor crashed). The holder is a TAB, not a person — keyed on holder **plus session id**, without which 25 tabs of the same cataloguer all inherited her lock at 25-way — and take-over is a compare-and-swap quoting the incumbent. Both expiry mechanisms are required: the acquire records `expired` when a lapsed lock is re-acquired, the sweep records it when nobody ever returns, and because the sweep DELETEs rather than marking, exactly one expiry row per lapse is structural — a soft delete there loses it._ _Amended: the record lock is a different mechanism at a different timescale from the write path's advisory lock — a human's intent to hold a record open for ten minutes — and shipped separately as **10b**. It is ADVISORY: `write()` does not read it and must not learn to, because an import, an overlay, a merge, a batch job and the phase-19 copy-forward all have to be able to write a record a cataloguer has open, and a lock that could refuse a save is one somebody has to override at 2am. `marc_record_locks` is one row per record ever opened, with liveness in the acquire predicate rather than an index predicate (Postgres refuses a `WHERE expires_at > now()` partial unique as non-IMMUTABLE, and a design where a sweep is what makes a lapsed lock acquirable freezes every record whose editor crashed). The holder is a TAB, not a person — keyed on holder **plus session id**, without which 25 tabs of the same cataloguer all inherited her lock at 25-way — and take-over is a compare-and-swap quoting the incumbent. Both expiry mechanisms are required: the acquire records `expired` when a lapsed lock is re-acquired, the sweep records it when nobody ever returns, and because the sweep DELETEs rather than marking, exactly one expiry row per lapse is structural — a soft delete there loses it._
_Depends on:_ 9.
...
_Depends on:_ 10.
_Accept:_ Import a 10,000-record `.mrc` through the API, export it, and the export re-parses to identical records. `catalog-verify` reports zero drift over those 10,000. A 25-row catalogue page issues **no TOAST reads**, measured with `pg_statio_user_tables`. _Amended: the instrument this line named cannot see the thing it asserts. TOAST fetches happen during output-tuple formation, outside the executor's buffer accounting, so `EXPLAIN (ANALYZE, BUFFERS)` prints no TOAST line at all and reported `Buffers: shared hit=185` for the very query `pg_statio_user_tables` showed performing 300 toast-block reads — an EXPLAIN-based test passes on the exact regression it exists to catch. The fixture has to be incompressible noise as well: the projector clamps `summary` to 2,000 characters and `search_text` to 8,000 and both columns are `SET COMPRESSION lz4`, so records created through the API never reach TOAST and a realistic fixture measures zero for both queries and asserts nothing._ The projector is fuzzed against structurally hostile records and never throws. `libriant_catalog_projection_drift_total` emitted and alerting.

**12. `packages/circ-policy` — the pure resolver.** Types, `specificity`/`rank` (mirroring the SQL generated column), `resolve`, `calendar` (`isOpenAt`/`nextOpen`/`openDaysBetween`, crossing into ICU **exactly twice per resolution** — instant → civil at the start, civil → instant at the end — with everything between on Hinnant `days_from_civil` day numbers; `formatToParts` on a cached formatter measured 3.74 µs against 0.009 µs for the integer path, **415× cheaper**, and `purity.test.ts` asserts that no other file so much as mentions `Intl`. Formatters are memoised in a module-level `Map`, the package's one piece of mutable state, so the civil corpus runs twice and is compared), `duedate`, `fines`, `blocks`, `resolveTemplate`, and `fixtures/resolution-vectors.json` (**630 vectors**, of which 192 are the selector sweep; `scripts/build-vectors.ts` regenerates them in 1.5 s, and `fixtures/` is in `.prettierignore` because the file is a cross-language contract read by `cargo test` and prettier's reflow would fail `format:check` on a file nobody edited). Greek movable-feast seeder (Julian Paschal, ~40 lines).
_Depends on:_ 1.
_Accept:_ Pure — no I/O, no `Date.now()`, asserted by test. All **630** vectors pass. The resolve half is six narrative cases **plus a sweep** — 64 selector combinations × 3 instants, 192 vectors taking every branch of the comparator — because six narrative cases let a resolver that ignores `shelvingLocationId` entirely pass the file. `vectors.test.ts` asserts three properties of the sweep itself: its context values are values the rules actually name (a sweep over unnamed values resolves to the wildcard 64 times and proves nothing), **every rule wins at least once** (a priority-50 override with no `effectiveTo` originally shadowed the maximum-specificity rule at every instant, making specificity 63 unreachable), and the `enabled = false` rule carrying priority 999 **never** wins. Expectations are generated by a second, deliberately different implementation in `scripts/build-vectors.ts` — specificity by bit shift, wall-clock→instant by scanning 113 UTC offsets, civil arithmetic by stepping through `Intl` — not by the code under test. Golden vectors cover Europe/Athens DST in both directions, a 2-hour loan starting 01:30 on a spring-forward night, the Greek split day (08:00–14:00 + 17:00–21:00), and Orthodox movable feasts 2026–2030. `rank()` equals the SQL column for all 64 selector combinations.

**13. Policy engine service.** `apps/api/src/policy` — _amended: ONE Nest module carrying two modules' worth of service, not the `policy` + `org` split this line proposed; `apps/api/src/org` does not arrive until phase 20a, for the 2.0 read surface_ — and **eighteen** tables, not five: the five policy tables plus `calendars`, `calendar_hours`, `calendar_exceptions`, `calendar_exception_hours`, `fixed_due_date_sets`, `fixed_due_date_ranges`, `circulation_rules`, `hold_policy_pickup_branches`, `notice_policy_templates`, `patron_categories`, `patron_category_limits`, `circulation_policy_version` and `circulation_settings`. That last one is new and deliberate: the `circulation_rules_enabled` flag of §8 risk 6 lives in `circulation_settings`, NOT in `tenant_settings` and NOT as a plan feature, because `EffectivePlanService.unlimitedPlan` rewrites every plan boolean to TRUE while `BILLING_ENABLED` is false — which would turn the matrix ON for every library, the exact inverse of "simple mode is the default" — and `PlanGuard` answers 402 Payment Required to a school library that has not failed to pay. `PolicySnapshotService` (Redis version mirror + pub/sub + process LRU), simple-mode façade over the wildcard rule, preview endpoint, `/circulation/explain`, `TenantClockService` + the ESLint ban on raw `Date` arithmetic in `circulation/`.
_Depends on:_ 12, 9.
_Accept:_ There are FOUR ways to retire the wildcard rule, not two, and the service refuses all four with a typed error: delete, `enabled = false`, an `effective_to` in the past, and — the sharpest — an `effective_from` in the FUTURE, which satisfies both partial unique indexes, reads as perfectly configured in the editor, and makes every checkout until that date raise `NO_MATCHING_RULE`. The guard is in the service and cannot be in the database: `circulation_rules_default_singleton` forbids a second enabled wildcard and says nothing about removing the last one, and a deferred `CONSTRAINT TRIGGER … AFTER TRUNCATE` is rejected outright on PG 16 (`FOR EACH ROW` unsupported for TRUNCATE, `FOR EACH STATEMENT` a syntax error), so a commit-time guard would close three doors of four and surface through Prisma as a generic transaction failure. `TRUNCATE` is left to phase 16's `REVOKE TRUNCATE`, backstopped by the version triggers' `OR TRUNCATE`, which makes every pod rebuild within a second and the desk refuse to lend rather than serve a matrix that no longer exists. A duplicate scope 409s from the partial unique, with the message keyed on the request's six selectors rather than on the constraint name — two wildcards violate `circulation_rules_scope_unique` FIRST, so `circulation_rules_default_singleton` never gets to report it. A policy write bumps `circulation_policy_version` **in the same transaction** and every pod serves the new snapshot within 1 s (pub/sub) or 30 s (TTL). Resolution p99 < 0.2 ms over a 500-rule snapshot. `/circulation/explain` returns `matchedRuleId`, `beatenRuleIds` and the calendar rolls applied.

**14. Patrons 2.0.** Service layer over the baseline tables: categories, patrons, cards, identifiers, addresses, relationships, blocks (with the `ON CONFLICT` recompute), messages, notes, merge, number minting (`UPDATE … RETURNING`, never `max()+1`), and `patron-data-map.ts`. _Amended: the GDPR erase/export was NOT extended here._ Phase 14 takes patron-referencing tables in `lbr2` from ONE (`loans.patron_id`) to eleven, and `check:dsar-coverage` — §5's compliance row promises it at phases 33 and 96 — does not exist. A gate written at phase 33 protects tables twelve onward; it cannot retroactively catch a table phase 14 forgot, it can only freeze the forgetting, because it will be written against whatever coverage map exists then. So what phase 14 owes and ships is the map that gate will consume: every table with a patron column, a verdict of `in_bundle` / `excluded` / `pending`, a required reason on the last two, and three tests holding it honest (every `lbr2` table with a patron column is in the map, every non-`in_bundle` entry has a reason, every entry claiming to exist does). `audit_log` is `excluded` because it is the record of what STAFF did, which the library needs precisely to show an erasure was carried out; `change_events` because the cascade to device replicas is phase 78 and there is no fleet to cascade to. The Article 15/20 subject-access bundle and the Article 17 erase themselves are written at **20b-ii**, which also takes the gate.
_Depends on:_ 13.
_Accept:_ Merge is transactional under sorted `patron:` locks; an old card barcode resolves through `merged_into_id` in **one hop**, never a chain; balances sum per currency. A concurrent block-recompute racing a desk transaction never aborts the desk transaction (25-way repro). `patrons_number_pattern_idx` with `text_pattern_ops` serves `LIKE 'M-2026-%'` (perf-13 preserved) — and the literal is stale: `infra/compose/docker-compose.prod.yml` now initdb's `--locale-provider=icu --icu-locale=el-GR --locale=C.UTF-8`, so `datcollate` reads `C.UTF-8` and any guard grepping for `el_GR` gets a false negative. The behaviour is identical under every one of them — a plain btree seq-scans (319 buffers) under libc `en_US.UTF-8`, libc `el_GR.UTF-8` and ICU `el-GR` alike, and still does with `enable_seqscan = off`, so there is no index path at all rather than a costing preference — so the test asserts the tenant collation is **not `C`** rather than a locale name, which is what keeps it true after the next locale change. Phase 13's `COLLATE "C"` precedent is NOT the thing to copy here: as the only index on the column it serves the `LIKE` and seq-scans `patron_number = $1`, while `text_pattern_ops` carries `=(text,text)` at btree strategy 3 and serves both, so there is no third index.
...
**16. Circulation engine, part 1.** `apps/api/src/circulation`: checkout, checkin with `CheckinDisposition`, renew (single/batch/all); `loan_events` with `occurred_at` **and** `effective_at`; `sync_client_changes` written in-transaction; `circulation_statistics` + `partition-maintenance`; and every lock taken through phase 10's `locks.ts`, which **sorts** keys by domain rank (policy < patron < bib < item) before acquiring — _amended: the helper and its CI grep SHIPPED IN PHASE 10, not here (see that phase's note); phase 16 is its first multi-domain consumer, not its author._
_Depends on:_ 14, 15.
_Accept:_ Policy resolved once and frozen; editing a rule afterwards provably does not change an open loan's due date or fine — proved by a RENEWAL taken after the edit (loan period halved 14 → 7 through the real trigger path, `circulation_policy_version` bumped, and the renewal still extends by fourteen days because it prices from the frozen snapshot), because asserting that a stored `due_at` is unchanged proves nothing: it is a column. One-open-loan-per-item holds under 25-way concurrent checkout of the same item. Replaying a `client_change_id` returns the stored response and re-applies nothing; a mismatched `request_hash` 409s. Zero deadlocks across a 4-way mixed workload for 10 minutes. Checkin p99 < 40 ms. The wire statement count is NOT asserted: it needs `pg_stat_statements`, which is not loadable on the cluster the suite runs against (`CREATE EXTENSION` succeeds on the Homebrew server and the view then reports "must be loaded via shared_preload_libraries"; `logging_collector` is off, so the statement log is unreadable too). It IS preloaded in the compose files, so a check against the container would falsely say it works — the two-cluster trap. What is asserted instead is sharper for what the budget protects: that a checkin is ONE transaction — proved by its change events sharing a single `commit_xmin` — plus the exact row footprint, so an N+1 inside a loop shows up as a count that scales with the fixture where a limit of twelve would not have noticed on a fixture of one.
...
_Depends on:_ 16.
_Accept:_ Property test: 10,000 random charge/pay/waive/refund/write-off sequences leave every reconciliation identity intact. Concurrency is **not** asserted in this phase: the per-tenant pool is clamped to one connection, so the property test's `Promise.all` over services queues rather than interleaves and a concurrency claim there would be vacuous. The genuinely simultaneous cases — the `fees_one_open_accrual_per_loan` upsert surviving a return that races the accrual sweep (the DATA-1 repro) and two desks closing one drawer — need their own harness on separate `pg` clients and are **phase 21's**, landing alongside the checkin that first calls `accrueWithin`. A reprint is byte-identical. A drawer close with a variance records it rather than adjusting. `libriant_circ_ledger_drift_total` emitted and alerting.
...
> **fourteen, and is not finished** — 20a, 20b-i, 20b-ii, 20b-iii and 20c–20l —
> because the original text was written before phases 10–18 existed and assumed a
> parity between 1.0 and 2.0 that those phases did not produce, and because
> everything the cutover would delete without replacing had to be built first:
> the importer's 2.0 write path (20c), circulation history (20d), the repair of
> 19b's upgrade (20e), the schema binding that decouples promotion from deploy
> (20f), the plan ceilings on the 2.0 write surface (20g), the routes with no 2.0
> equivalent (20h) and the web repoint (20i–20l). The measurements behind every
> change here are in the divergence log (`README.md`, one section per sub-phase). Phase NUMBERS remain stable identifiers; the letters
...
**42** assertions, not ~40 — **44 since phase 20e**, which repaired what this
phase froze. The `policy_snapshot` 19b wrote
(`{migratedFrom, loanPeriodDays, maxRenewals, finePerDayCents, currency}`) has
**none of the five** fields `readPinnedPolicy` requires, so every migrated loan
threw `PinnedSnapshotError` on its detail screen, on renew and on checkin, and
the overdue sweep counted it `refused` and charged nothing; 20e pins a real
named resolution through `@libriant/circ-policy` instead. E04 could not catch
any of it — it asserted only `policy_snapshot IS NOT NULL AND <> '{}'`, which the
broken blob satisfies — so it now asserts the five things the READER checks,
`E04b` is new beside it, and `E04c` asserts every migrated patron has an account.
`scripts/seed-v1-fixture.ts` builds the fixture in
two profiles (`ci`, `acceptance`); CI seeds and upgrades it on every commit.
There is no `--commit` in this phase by design.
_Depends on:_ 19a.
_Accept:_ All 44 assertions hold on the `ci` profile — row counts exact,
...
the desk; and the import engine's 2.0 write path — **which left this phase and shipped on its own: 20c writes `book`, `book_copy` and `member` through `BibWriteService`, `ItemsService` and `PatronsService`, so an imported record gets the version history, projection row and audit trail a raw insert would not, and `processImportJob` selects the engine on `tenant_schema_state.schemaMajor`; 20d adds `loan`, `reservation` and `fine`, written directly rather than through `CheckoutService` / `HoldsService.place` / `FeesService.settle`, because those services mean "lend this book now" and cannot express `dueAt`, `returnedAt`, `renewalCount` or a closed loan. `author` stays refused by name until the authority store at phase 45.** Move the five integration
...
> **Found with no owning phase (20j's survey, amended by 20k).** Four 1.0
> capabilities have neither a named 2.0 successor nor a written decision to lose
> them, and the cutover deletes the only implementation of each. **Void a fee**
> (1.0 `POST /fines/:id/void`) — phase 18 shipped charge, pay, waive, write-off,
> refund, drawer and receipts and named no cancellation. **Fulfil one reservation
> by id** — no 2.0 route takes a hold id and produces a loan; the 2.0 path is
> item-keyed `holds/fetch` then `circulation/checkout`. **Expire ONE reservation**
> — 2.0 has only the two tenant-wide sweeps. **Resolve a patron by MEMBER
> NUMBER** — the checkout screen's scan-to-find; 2.0 resolves a `patron_cards`
> barcode and nothing resolves `patron_number`. A fifth gap runs the other way:
> **custom fields on a bibliographic record**. `bib_records.custom_fields` exists
> and `BibProjectionService` lists it in `NOT_THE_PROJECTORS` so it survives
> re-projection, but nothing writes it, no route reads it, the v2 importer errors
> on a `book` row that carries one, and the field-definition controller is still
> on the 1.0 client — so no phase owns the surface.
>
> Two items from the same survey are closed rather than outstanding.
> **Per-fine settlement** is a deliberate phase-18 decision — 2.0 settles a
> patron's balance by amount, not one fine by id — so that screen is a rewrite,
> not a repoint. **`itemTypeId` had no list route**, which blocked add-a-copy;
> 20k added `GET /t/:slug/org/item-types` beside `branches` and `locations`,
> under the existing `cat.bib.read` key.
...
> so the five 1.0 author routes do not come back as such; contributors live inside
> the MARC record until the authority store lands at phase 45. Author SEARCH does
> have a 2.0 answer and got one in phase 20h: `GET /catalog/bib/contributors?q=`
> returns the distinct `browse_author` values this library has already used,
> folded before comparison like every other search here, so a cataloguer stops
> retyping — and re-spelling — a name the catalogue already holds. Author CREATE
> has no equivalent and gets none before 45: 1.0's `AuthorPicker` searches an
> `authors` table by id and POSTs a row, and what 2.0 will consume is a name
> typeahead rather than an entity picker, so repointing it is a UI change that
> belongs with the web work. Claims-returned stays assigned
> to phase 21 and its loan-detail button goes with it. **Mark-lost does not**: a
> library that upgrades and can no longer record a lost book has lost a
> circulation capability rather than a screen, so phase **20h** built
> `POST /t/:slug/circulation/loans/:id/declare-lost` on 1.0's own
> `circ.loan.mark_lost` key (no role-template change, and the key stops being
> orphaned). The loan closes without a return under `loans_closed_consistency`,
> the reader keeps their link because anonymisation belongs to the return, the
> copy moves to `missing` through phase 15's single status writer (`item_status`
> has no `lost` value), the replacement charge goes through the real ledger in
> its own transaction, and `LoanEventKind` gained `declared_lost` in its own
> migration. Only the fee's PRICING stays phase 21.
...
and their specs; repoint whatever staff UI is still on 1.0 — the catalogue family was repointed
ahead of the cutover, as independently shippable phases (20i the list, 20j the
simple MARC editor, 20k the record/copies/cover/spine-label screen, 20l create),
so what remains here is the members, loans+reservations and fines families plus
`catalog/new/BookForm.tsx`, which survives only because the onboarding wizard
mounts it and dies with the 1.0 modules; **no `V2_SCHEMA` rename and no `lbr2.` literal rewrite — phase 20f did both, and
doing it the way this line said would have been destructive**: `withV2Schema`
feeds the same constant to `prisma migrate deploy` and four callers deploy both
migration folders, so a constant reading `'public'` points both at one schema
where 13 physical names collide and the two `_prisma_migrations` ledgers become
one. 20f replaced the constant with `v2SchemaFor(schemaMajor)` at every runtime
call site plus `search_path = lbr2, public` — the ORDER load-bearing, because
nine names collide and Postgres silently ignores a schema in `search_path` that
does not exist, so one string serves promoted and unpromoted tenants alike —
deleted **162 `lbr2.` qualifications from 33 runtime files**, and added
`check:v2-schema-literals`, which fails on a single re-introduced `lbr2.` that
would otherwise pass every test in the repository, because every test database is
unpromoted. What this phase still owes is `withV2Schema` itself, which feeds the
constant to `prisma migrate deploy`: against a promoted tenant it CREATEs an
empty `lbr2`, replays the whole 2.0 baseline into it and reports success —
unreachable from the running product, reachable from `tenant:migrate`,
`tenant:create` and the maintenance processor; retire the 1.0 locale keys that name a concept 2.0 drops. This is a DELTA
discovered during each screen's repoint, not a rewrite performed before them: the
1.0 namespaces are CATALOGUE vocabulary, not 1.0 vocabulary, so a 2.0 screen says
the same words. Measured — the catalogue list (20i) needed **zero** new keys in
either locale, reusing `catalog.title`, `columns.title`, `columns.author`,
`columns.copies` and `common.search.minChars` as they stood; the simple editor
(20j) needed **five** (`editionLabel`, `saved`, `noChanges`, `fieldNotOnRecord`,
`fixedFieldsLater`). The keys that retire are the ones naming a concept 2.0 drops;
the keys that appear are for concepts it adds; both are found by repointing the
screen, not by guessing in advance.
...
**21. Circulation engine, part 2 + the unified desk.** Declare-lost's fee **pricing** from `lost_item_fee_policies` through the loan's pinned snapshot — the route itself (`POST /t/:slug/circulation/loans/:id/declare-lost`, on the existing `circ.loan.mark_lost` key, with `LoanEventKind.declared_lost`) shipped in phase 20h and charges only when an amount is explicit, saying in words which happened, because a cheaper fallback would put a number nobody chose onto a reader's account — mark-found with refund, claims-returned / claims-never-borrowed, recall, transfers with the transit desk, overrides with reason codes and `override_permissions`, fast-add; the unified desk UI (patron pane + item pane + disposition card + block banner + due-date explainer) on the responsive layer.
...
1. **The cutover corrupts a real library's data.** _Mitigation:_ one atomic transaction with a 40-assertion verifier inside it; rollback is `scripts/tenant-rollback-v2.ts`, which sends the six extensions back out of the promoted `public` before renaming `v1_archive` over it, and which refuses to run twice — a bare rename after the promotion SUCCEEDS and cascades away the archive's own `members.email`, its four 1.0 trigram indexes and every extension; CI runs the whole upgrade against a generated v1 fixture on every commit; the demo tenant is cut over and exercised through the UI before any customer; a named first library with a rehearsed cutover date and a documented rollback decision point.

2. **Timestamp conversion silently shifts every date by three hours.** Prisma's generated `ALTER COLUMN … TYPE timestamptz` casts through the session TimeZone. _Amended after measuring it in the UTC-session interlude (between phases 17 and 18): both halves of this risk were wrong._ **The deployed Postgres session is UTC, not Athens** — `postgres:16-alpine` ships no `TZ`, `docker-compose.prod.yml` passes none, and the Berlin host zone in the RUNBOOK is the app container's clock, not the Postgres session (one `SHOW TimeZone` against production is still owed, to be certain rather than persuaded). And the live instance of this risk was never a migration: it was the **driver**. `@prisma/adapter-pg@7.9.1` requires a UTC session and does not say so — it writes a `timestamptz` as a naive local timestamp with no zone, and reads one back by overwriting the rendered offset with `+00:00`. The two errors are equal and opposite, so a Prisma-only round trip agrees with itself and the defect is **invisible from inside Prisma**; what is wrong is the instant physically stored (by the session's offset at that instant, so not even a constant), any predicate mixing a Prisma-written column with a server-side `now()`, and the spring-forward hour, whose instants the application cannot store at all. The only non-UTC cluster is a developer's local Postgres — which is exactly where the test suites and `pnpm tenant:smoke` run — so the product was correct **by accident**. _Mitigation:_ every conversion hand-written `USING col AT TIME ZONE 'UTC'`; `check:migration-safety` fails any bare cast; the verifier asserts every converted instant is within 1 s of the v1 value read as UTC; **`ALTER DATABASE … SET TimeZone TO 'UTC'` at every `CREATE DATABASE` site, before the migrations** (it reaches `prisma migrate deploy`, whose Rust schema engine ignores libpq environment variables, plus `psql`, `pg_dump` and the smoke harness, which no pool option can); `options: '-c timezone=UTC'` on all three Prisma pools as the backstop for a database somebody forgot or a customer's own cluster — **never on the URL**, because `URLSearchParams` re-serialises the space as `+` and libpq answers `FATAL: unrecognized configuration parameter "+timezone"`; `check:session-timezone` (four rules, each proved to fire) and `session-timezone.spec.ts`, which makes its own `Asia/Kolkata` database so it discriminates identically on a developer cluster and on CI; and `scripts/tenant-timezone-audit.ts --pin`, which refuses a database that is both populated and skewed rather than inventing row values.
...
_Runners-up worth naming:_ the sizing of the merged stack against a shared 128 MB `shared_buffers` box (mitigated by profiles-off-by-default and a per-phase deployment budget as an acceptance criterion); `@node-saml/node-saml`'s transitive XML tree, the dependency with the highest ongoing maintenance obligation in the design; and the marketing site's public-catalogue claims that become untrue at phase 31 — **eleven strings per locale, not four; see the count in the M2 note** — one of which, at line 806, would become an untrue _security_ claim.
