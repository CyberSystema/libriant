...
| `worker`           | 3005 (internal, health only) | data        | always      | BullMQ consumer: six queues — `notices`, `catalog`, `search`, `report`, `acquisitions`, `webhook`.                                                                                                                                                                                                                                               |
...
| `site`             | 3004 (internal)              | edge        | always      | Marketing site.                                                                                                                                                                                                                                                                                                                                  |
...
Written **inside the same transaction as the effect**, retained 180 days. `apps/web/lib/offline-queue.ts`'s `MAX_QUEUE_AGE_MS = 18h` — chosen with 6 h of margin under `IdempotencyInterceptor.RESULT_TTL_SEC = 24h` in Redis — is deleted. The Redis interceptor stays for the interactive HTTP path and gains a **fail-closed** variant for money-moving routes; its fail-open default is defensible for a checkout and indefensible for an approved payment.
...
_Depends on:_ 1.
_Accept:_ Property test over ≥5,000 records (LC + real ABEKT/Koha/Aleph/Evergreen): `parse(serialize(parse(b))) ≡ parse(b)` for all, and `serialize(parse(b)) === b` byte-for-byte for ≥98 %, residue documented per record. Every serialized leader matches `^.{10}22.{9}4500$`. MARC-8 round-trips LC's codetable vectors including combining-mark-before-base reordering and the U+FE20–FE23 pairs; a Greek 245 followed by a Latin 260 both decode correctly (designation reset). `applyOps` inserting a paired field renumbers every `$6` on both sides. `check:greek-folding` still green.

**8. Format definitions and the validator.** `scripts/gen-marc-schema.ts` → committed Avram JSON for MARC21 bib/authority/**holdings (008 included)** and UNIMARC bib/authorities; the layered loader (shipped ← tenant override); the validator; RDA/AACR2/ISBD/UNIMARC rule packs as data; `validateDelta`. New gate `check:marc-schema`.
_Depends on:_ 7.
...
**9. The 2.0 baseline migration.** One squashed migration creating **every cross-domain table** in §3 — branches (with the cycle trigger, ISIL, **timezone**), service points, shelving locations, calendars, item/material types, patron categories, patrons + cards, holdings, items, the MARC store trio, loans, holds, fees + ledger, notice tables, bookings, roles/permissions, `change_events` + generated triggers, partitioned `audit_log`, `sync_client_changes`, `custom_field_values`, settings singletons — plus `btree_gist`, `btree_gin`, `record_version_seq`, `default_toast_compression=lz4`. **No services yet.** `check:changelog-coverage` and `check:schema-conventions` land here.
_Depends on:_ 2, 8.
...
| **MARC-8 EACC (CJK)**                                  | Ship the codec structure and Latin/Greek/Cyrillic/Hebrew/Arabic tables; EACC is a lazily-loaded stub raising "CJK MARC-8 not supported, re-export as UTF-8". No Greek library exercises it and it is the least-testable path in the highest-risk component.  |
...
packages/shared/src/callnumber/{ddc,lcc,udc,nlm,alphanum,local}.ts
...
packages/shared/src/callnumber/__fixtures__/{ddc,lcc,udc,nlm}.json   # ≥2000 real numbers/scheme
...
scripts/check-greek-folding.mjs                     # NEW GATE
...
- `packages/shared` gains a `test` script: `node --import tsx --test "src/**/*.test.ts"` (matching `packages/ui`, not vitest — this is a pure library with no DB fixtures), plus the `build`/`typecheck`/`lint` scripts turbo requires.
...
- **`check:greek-folding` (new)** — runs the fixture list through the TypeScript implementation and through a Postgres `IMMUTABLE` SQL wrapper (`libriant_fold_greek(text)`, created in the gate's throwaway database) and fails on any divergence. The gate is written now with two runtimes and grows two more later — the OpenSearch analyzer chain in phase 82 and the Rust core's `cargo test` fixture in phase 77 — because that is the only thing standing between a name that sorts in one place and cannot be found in another.
