/**
 * The 1.0 catalogue → loan → reservation → custom-collection workflow.
 *
 * This is the whole of the original `smoke-test.ts`, moved into one module when
 * phase 9 split the runner. Nothing about it changed: it is the 1.0 schema's
 * smoke test, it still runs against `public`, and phase 20 deletes it along with
 * the tables it exercises.
 */
import { Prisma, type TenantPrismaClient } from '../../src';
import { ok, note, expectError, cleanup, type SmokeModule } from './_lib.js';

// The client and the connection string are the RUNNER's business now; this
// module receives one and does not own its lifetime.
async function run(db: TenantPrismaClient): Promise<void> {
  ...
  console.log('\n1.0 workflow complete ✓');
}

export const v1Workflow: SmokeModule = {
  name: 'v1-workflow',
  describes: 'the 1.0 catalogue, loans, reservations and custom collections (deleted by phase 20)',
  run,
  reset: cleanup,
};
